...
class RoomWsClient {
  ...
  private startHeartbeat() {
    ...
    this.heartbeatTimer = setInterval(() => {
      ...
      try {
        ...
        logger.warn('wsClient', 'ping send failed', {error: e});
      }
    }, 6_000);
  }
    ...
    try {
      ...
      socket.onopen = () => {
        ...
        try {
          ...
          logger.warn('wsClient', 'send hello failed', {error: e});
        }
      ...
      socket.onmessage = event => {
        try {
          ...
          logger.warn('wsClient', 'onmessage parse failed', {error: e});
        }
      };
      ...
      socket.onerror = ev => {
        logger.warn('wsClient', 'socket error', {error: ev});
      };
    } catch (e) {
      logger.warn('wsClient', 'connect failed', {error: e});
      this.scheduleReconnect();
    }
  }
  ...
  sendRead(ts: number): boolean {
    ...
    try {
      ...
      logger.warn('wsClient', 'sendRead failed', {error: e});
      this.pendingReadTs = Math.max(this.pendingReadTs || 0, ts);
      return false;
    }
  }
  ...
  sendContainer(payload: OutgoingContainerPayload): boolean {
    ...
    try {
      ...
      logger.warn('wsClient', 'sendContainer failed', {error: e});
      this.pendingContainers.push(payload);
      return false;
    }
  }
  ...
  sendRtc(signal: OutgoingRtcSignal) {
    ...
    try {
      ...
      logger.warn('wsClient', 'sendRtc failed', {error: e});
      this.pendingRtc.push(signal);
    }
  }
...
export function sendRoomContainer(payload: OutgoingContainerPayload): boolean {
  ...
  if (!client) {
    logger.warn('wsClient', 'sendRoomContainer: no client for room', {data: {roomId: payload.roomId}});
    return false;
  }
...
export function sendRoomRtcSignal(signal: OutgoingRtcSignal): void {
  ...
  if (!client) {
    logger.warn('wsClient', 'sendRoomRtcSignal: no client for room', {data: {roomId: signal.roomId}});
    return;
  }
